...
if($output->data['type']==='feuille-de-route'){
    $stm = $dbh->prepare( "SELECT DISTINCT `feuille-de-route`.id, chapitre, titre, status, delai FROM `feuille-de-route`
    ...
    $stm = $dbh->prepare("SELECT
                ...
                GROUP_CONCAT(titre ORDER BY exercices.id SEPARATOR ';') as exercices
            FROM `users`
